(function (global, factory) {
	...
	(global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.CollectorSearch = factory());
	...
	function SsSearchWrapper(attrs) {
	  ...
	  if (!lodash.isInteger(perPage) || perPage < 1) {
	    perPage = 200;
	  }

	  var search = function search(query, page) {
	    var results = lib.search(documents, searchKeys, query);
	  ...
	  var advancedSearch = function advancedSearch(query, page, options) {
	    ...
	    var results = lib.search(matchingDocuments, searchKeys, query);
	    return sortAndPaginate(results, page, perPage);
	  };
	  ...
	  var sortAndPaginate = function sortAndPaginate(results, page, perPage) {
	    ...
	    return lodash.chain(results).sortBy("order").drop((pageNumber - 1) * totalPerPage).take(totalPerPage).value();
	  };
	  ...
	  var normalizePageNumber = function normalizePageNumber(page, perPage, totalResults) {
	    if (!lodash.isInteger(page) || page < 1) {
	      return 1;
	    }
	...
	function CollectorSearch(attrs) {
	  ...
	  if (lodash.isEmpty(searchWrapper)) {
	  ...
	  var extractOptionsFromQuery = function extractOptionsFromQuery(query) {
	    ...
	    if (!lodash.isString(remainingQuery)) {
	      return {
	    ...
	    lodash.forEach(matches, function (match) {
	      var fieldAndValue = lodash.chain(match).replace(/\s/g, "").split(":").value();

	      options[fieldAndValue[0]] = fieldAndValue[1];
	      remainingQuery = lodash.chain(remainingQuery).replace(match, "  ").replace(/\s{2,}/g, " ").trim().value();
	    });
