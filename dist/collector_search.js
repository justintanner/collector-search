(function (global, factory) {
	...
	var CollectorSearch = function CollectorSearch(attrs) {
	  ...
	  var filterDocuments = function filterDocuments(searchDocuments, query, options) {
	    ...
	    lodash.each(advancedFilterFunctions(options), function (filterFunction) {
	  ...
	  var extractOptionsFromQuery = function extractOptionsFromQuery(query) {
	    ...
	    lodash.each(matches, function (match) {
	      var fieldAndValue = match.replace(/\s/g, "").split(":");
	      options[fieldAndValue[0]] = fieldAndValue[1];
	      remainingQuery = remainingQuery.replace(match, "  ").replace(/\s{2,}/g, " ").trim();
	    });
	  ...
	  var sortAndPaginate = function sortAndPaginate(results, page, perPage) {
	    ...
	    return lodash.chain(results).sortBy(orderBy).drop((pageNumber - 1) * totalPerPage).take(totalPerPage).value();
	  };
	  ...
	  var advancedFilterFunctions = function advancedFilterFunctions(options) {
	    var validKeys = lodash.reject(Object.keys(documents[0]), keysToExclude);

	    return lodash.chain(options).map(function (value, key) {
	      if (isNumberRange(value)) {
	        return function (document) {
	          ...
	          var number = lodash.parseInt(document[key]);
	  ...
	  var isNumberRange = function isNumberRange(string) {
